...
export class CvService {
  ...
  async createUserCv(userId: number, cvDto:CreateCvDto): Promise<Cv> {
    ...
    if (cvDto.educations) {
      ...
      if (cvDto.templates) {
        const template = await this.templateRepository.findOne({ where: { id: cvDto.templateUuid } });
        if (!template) {
            throw new NotFoundException(`Template with UUID ${cvDto.templateUuid} not found.`);
        }
  ...
  async updateUserCv(userId: number, cvId: number, updateCvDto: UpdateCvDto): Promise<Cv> {
    ...
    if (updateCvDto.professionalSummary) existingCv.professionalSummary = updateCvDto.professionalSummary;
    // Update educations
    if (updateCvDto.educations) {
      try {
      const educations = updateCvDto.educations;
      let previousEdu = await this.educationRepository.find({ where: { cv: { id: cvId } } });
      ...
      for (const education of educations) {
        if (education.id) {
          ...
          const newEducation =  this.educationRepository.create({...education, cv_id:cvId});
    ...
    if (updateCvDto.experiences) {
      ...
      let previousEx = await this.experienceRepository.find({ where: { cv: { id: cvId } } });
      ...
      for (const experience of experiences) {
        if (experience.id) {
          ...
          const newExperience =  this.experienceRepository.create({...experience, cv_id:cvId});
    ...
    if (updateCvDto.socials) {
      ...
      for (const social of socials) {
        if (social.id) {
          ...
          const newSocial =  this.socialRepository.create({...social, cv_id:cvId});
    ...
    if (updateCvDto.skills) {
      ...
      for (const skill of skills) {
        if (skill.id) {
          ...
          const newSkill =  this.skillRepository.create({...skill, cv_id:cvId});
    ...
    const updatedCv = await this.cvRepository.findOne({
      ...
      relations: { experiences: true, educations: true, socials: true, skills: true }
    });
