...
let allEducations = reactive([]);
let isOpen = ref([]);

watch(resumeStore.currentResume,async(newVal) => {
    if(resumeStore.addedEducations.length > 0){
        allEducations = newVal.educations.concat(resumeStore.addedEducations)
    }
    else {
        allEducations = newVal.educations
    }
}, { deep: true, immediate: true })

watch(resumeStore.addedEducations,async(newVal) => {
    if(resumeStore.currentResume.educations.length > 0) {
        allEducations = newVal.concat(resumeStore.currentResume.educations)
    }
    else {
        allEducations = newVal
    }
}, { deep: true, immediate: true })

async function removeEducation(educationId, index) {
